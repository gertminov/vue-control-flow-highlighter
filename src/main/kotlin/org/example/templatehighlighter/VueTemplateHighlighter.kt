...
class VueTemplateHighlighter : Annotator {
    companion object {
        ...
        private val LOG = Logger.getInstance(VueTemplateHighlighter::class.java)
    }
    ...
    override fun annotate(
        ...
        if (element is XmlTag && element.name == "template") {
            ...
            val startTag = element.firstChild
            val startRange =
                TextRange(
            ...
            if (lastChild.text == ">") {
                val rangeStart = lastChild.textRange.endOffset - "</template>".length
                val closeRange =
                    TextRange(
                        rangeStart,
